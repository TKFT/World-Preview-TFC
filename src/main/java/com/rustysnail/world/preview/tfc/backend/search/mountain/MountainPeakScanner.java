...
public class MountainPeakScanner
{
    ...
    public MountainPeakScanner(long seed, HeightSampler heightSampler, MountainSearchConfig config, Callback callback)
    {
        this.seed = seed;
        ...
        try
        {
            ...
            for (int x = cx - r; x <= cx + r; x += coarseStep)
            {
                for (int z = cz - r; z <= cz + r; z += coarseStep)
                {
                    ...
                    int h = heightSampler.surfaceY(x, z);
                    samplesChecked++;
